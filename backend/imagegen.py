...
def build_image_prompt(image_prompt: str, style_desc: str, rel_out: str,
    ...
    if has_character_ref:
        explainer = (
            ...
            "100% 동일하게 유지하고 시트의 정체성을 그대로 이어간다.\n"
            "- 2번(마지막) 세모지 베이스: 이미지 전체의 그림체·색감 기준(그림체와 색감만 참고, "
            "인물은 1번 시트의 인물로 그린다)."
        )
    else:
        explainer = (
            ...
            "화면에는 배경과 사물만 담는다(무인 장면, 오직 공간과 오브젝트)."
        )
    return (
        f"{style_desc}\n\n## 장면\n{image_prompt}\n\n{explainer}\n\n## 생성 지시\n"
...
def build_character_prompt(name: str, looks: str, rel_out: str) -> str:
    ...
    return (
        ...
        f"- 헤어와 의상만 변경: {looks}\n"
        f"비율은 첨부한 1번 이미지가 정한다. 화면은 일러스트만으로 채운다. "
...
def build_asset_prompt(image_prompt: str, style_desc: str, rel_out: str,
    ...
    return (
        ...
        f"화면에는 배경과 사물만 담는다(무인 장면, 오직 공간과 오브젝트).\n\n## 생성 지시\n"
        f"image_gen 도구로 위 아트스타일을 적용한 이미지 1장을 생성해 현재 폴더의 {rel_out} 로 저장.\n"
        f"비율은 첨부 이미지가 정한다. 화면은 일러스트만으로 채운다. 저장되면 'OK'만 답해."
    )
...
def build_element_layer_prompt(name: str, location: str, style_desc: str, rel_out: str,
    ...
    excl = (f"단, 다음은 별도 레이어이므로 이 컷아웃에서는 그린 배경으로 둔다: {', '.join(others)}.\n"
            if others else "")
    return (
        f"{style_desc}\n\n## 레이어 분리 — 단일 요소(투명 컷아웃)\n첨부한 씬 이미지를 레퍼런스로 사용한다.\n"
        f"이 씬에서 '{name}'({location})를 단독 피사체로 다시 그린다. "
        f"가장 중요한 규칙: 원본 씬에서 이 요소가 차지하는 **정확히 같은 좌표와 같은 크기**를 그대로 보존한다 — "
        f"원본과 동일한 위치·동일한 스케일로 제자리에 그린다. "
        ...
        f"오직 균일한 단색 크로마키 그린(#00FF00) 한 색으로만 덮는다. 피사체만 그리고 나머지는 전부 순수 그린이다.\n"
        f"화면은 그림만 담는다."
    )
...
def build_layer_prompt(layer_kind: str, style_desc: str, rel_out: str) -> str:
    ...
    if layer_kind == "character":
        body = ("등장 인물(캐릭터)들만 동일한 포즈·외형·위치로 단독 컷아웃으로 다시 그리고, "
                "인물 밖 전 영역은 오직 균일한 단색 크로마키 그린(#00FF00) 한 색으로만 덮는다.")
    else:  # background
        body = ("배경·환경·공간만으로 화면을 채운다(무인 장면). 인물이 있던 자리는 주변 배경을 "
                "자연스럽게 이어 그려 메운다.")
    return f"{head} {body}\n화면은 그림만 담는다."
