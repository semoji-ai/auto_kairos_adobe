...
# 모델별 프롬프트 스타일 힌트(LLM에 주입) — 같은 씬도 모델마다 잘 먹는 프롬프트가 다름.
_MODEL_PROMPT_STYLE = {
    "seedance": ("Seedance 계열 — 영화적 카메라 무브(느린 푸시인·팬)와 피사체의 자연스러운 미세 동작, "
                 "빛·분위기 묘사를 한 문단으로. 카메라→피사체 동작→분위기 순."),
    "kling": ("Kling 계열 — 간결하고 동적인 액션 중심. 무엇이 어떻게 움직이는지 한두 문장으로 또렷하게, "
              "카메라 무브 1개만."),
}
...
def build_video_prompt(proj_dir: Path, scene: dict, model: str, *, on_event=None) -> dict:
    ...
    prompt = (
        ...
        "카메라 무브는 절제(느리게). 영어로, 프롬프트 텍스트만 출력(따옴표·설명 없이)."
    )
