...
# 표현(연출) 문법 — 이 비트를 '무엇으로' 보여줄지. 카메라는 여러 수단 중 하나일 뿐(기계적 푸시인 방지).
_DIRECTION_GRAMMAR = (
    "## 표현 판단(중요 — 카메라 이전에 '무엇으로 보여줄지'부터 정한다)\n"
    "우선순위:\n"
    "1) 피사체 액션·제스처·표정 (사람/캐릭터가 프레임에 있으면 최우선): 내레이션의 태도·감정을 몸으로. "
    "예) 부정·반전('아니다') → 고개를 젓거나 손사래·뒤로 물러나는 '아니다' 제스처, 놀람 → 표정 변화, "
    "결심 → 자세 변화. 반전이라고 카메라 푸시인부터 떠올리지 말 것.\n"
    "2) 프레임 내 변화: 시선 이동, 특정 대상을 손으로 가리킴, 빛·그림자의 미묘한 변화.\n"
    "3) 카메라 무브: 인물 액션으로 표현이 안 될 때만(문서·정물의 디테일 주목=느린 푸시인, "
    "규모=풀백, 이동·시간=팬). 인물이 있으면 대개 3)보다 1)이 낫다.\n"
    "- 설명·정적이면 미세 동작(숨·눈 깜빡임)만, 카메라 고정.\n"
    "- 새 그래픽·텍스트·기호(X표시 등)를 화면에 그려 넣으라고 요청하지 말 것 — 그건 AE 모션그래픽 몫. "
    "i2v는 자연스러운 동작·표정·카메라로만 표현한다.\n"
    "- 확신 없으면 절제(미세 동작 + 고정)."
)
