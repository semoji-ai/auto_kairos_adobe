...
| ⑩ 스토리보드 | `storyboard` | final + references + images | `storyboard.json` (+ 패널 렌더) | codex(텍스트) |
...
### 3.4 스토리보드 (⑩) — v4에 없음, 자체 정의
- `storyboard.json`: 씬별 `{sceneNumber, narration, 연출노트, ref_image, image_prompt}`. M2 scenes.json + references + 모션그래픽 메타 결합.
- 패널: M2 씬 목록 렌더를 확장해 썸네일(ref 이미지) + 연출노트 표시.
...
- **M3c — 스토리보드 (⑩)**: storyboard 스킬 + 패널 썸네일/연출 렌더.

> 권장 순서: **M3a 먼저** → (imagegen PoC) → M3b → M3c. 각 플랜은 그 자체로 동작하는 산출물.
