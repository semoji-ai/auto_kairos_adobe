...
- **재사용**: 콘텐츠 엔진은 auto_kairos_v4(PD 스킬 + projects 스토어 + codex imagegen). auto_kairos_v3는 미변경.
...
### FR-3 PD Chat / 스킬 실행 [MVP]
- 자연어 지시 → 백엔드 → `codex exec` → 결과 요약 + 생성 아티팩트 목록.
- 작업 모드: 원고검토 / 씬분해 / 이미지생성 / 이미지수정 / AE컴프 / 상태점검.
- **제약**: 채팅은 AE 타임라인을 직접 수정하지 않는다(컴프 변경은 별도 승인 버튼).
- **수용 기준**: "씬 분해" 지시 → `codex exec`가 units.json 생성, 패널에 결과 요약 표시.
