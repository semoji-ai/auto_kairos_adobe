...
- **카드형 레이아웃을 고르면 그 레이아웃의 데이터 필드를 반드시 함께 채운다** — 데이터가 없으면 그 레이아웃은 빈 화면이 되므로, 데이터를 못 채울 상황이면 차라리 `cinematic`으로 둔다.
  - **`metric_spotlight`** — `value`(핵심 수치 한 개, 예 `"96%"`, `"340kg"`)와 `label`(그 수치를 설명하는 한 줄). 둘 다 필수.
  - **`quote`** — `quote_text`(내레이션에 실제로 있는 인용문)와 `quote_who`(발언자). quote_text 필수.
  - **`items_list`** — `headline`(제목)과 `items`(2~5개 항목 문자열 배열). items 필수.
  - **`headline_only`** — `headline`(핵심 한 줄), 필요 시 `sub`(보조 한 줄). headline 필수.
